...
%%%%%% Pressure Solve %%%%%%%%%%%%%%%%%%%%%%%%%%%
if Option_PressureSolve
    % This is an outdated method. The direct solve used below is more
    % accurate and there is no chance for a poor resolution solve.
...
%%%%%% Direct Solve %%%%%%%%%%%%%%%%%%%%%%%%%%%
% A linear solver for flowrate in a network without any loops ( which are 
% not possible with the current vessel tree anyway). Directly solves for
% flows in each segment using conservation of mass equation.
    
    disp('Directly solving flows in Arteries')
    [FdotArt, SplitArt] = velocitySolver1DFunctionDirectSolve(...
        Vessel1,MdotArt,InletPoints,InletFlows,[],[]);
    disp('Directly solving flows in Veins')
    [FdotVein, SplitVein] = velocitySolver1DFunctionDirectSolve(...
        Vessel2,MdotVein,[],[],OutletPoints,OutletFlows);
    % Solves the flowrates within the arteries and veins. Flowrates are
    % generated with two values. The first column refers to
    % the flowrate closer to the connecting node. The second column refers
    % to the flowrate closer to the current node (node N). Positive flows
