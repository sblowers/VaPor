...
GM_WM_Convert2 = GM_WM_Convert + numel(GM_WM(GM_WM));
RowConvert = [RowConvert; RowConvert];
...
MTT = (2*Porosity*VoxelSize^3)./(MassFlow/Rho_b); % s
VolFrac = [0.1,0.8,0.1];
MTT1 = VolFrac(1)*(2*Porosity*VoxelSize^3)./(MassFlow1/Rho_b); % s
MTT2 = VolFrac(2)*(2*Porosity*VoxelSize^3)./((Perfusion*VoxelSize^3)/Rho_b); % s
MTT3 = VolFrac(3)*(2*Porosity*VoxelSize^3)./(MassFlow2/Rho_b); % s
% MTT = 1./(MassFlow/rho./(porosity*h^3))*3; % s
% Guess for mean transit time for voxels given porosity.

Tree = cell(2*NumGM_WM+NumArt+NumVein,1);
...
for ArtLoop = 1:NumArt
    ...
    if SplitArt(ArtLoop,1)~=0
        for N = 1:size(Vessel2Volume1{ArtLoop},1)
                if Vessel2Volume1{ArtLoop}(N,4) ~= 0;
                    Tree{ArtLoop+2*NumGM_WM}(end+1,:) = [GM_WM_Convert(Vessel2Volume1{ArtLoop}(N,1),Vessel2Volume1{ArtLoop}(N,2),Vessel2Volume1{ArtLoop}(N,3))...
                        ,SplitArt(ArtLoop,2)*Vessel2Volume1{ArtLoop}(N,4),0];
                    Tree{Vessel1(ArtLoop,7)+2*NumGM_WM}(end+1,:) = [GM_WM_Convert(Vessel2Volume1{ArtLoop}(N,1),Vessel2Volume1{ArtLoop}(N,2),Vessel2Volume1{ArtLoop}(N,3))...
    ...
    if FdotArt(ArtLoop,1) <= 0 
        ...
        if abs(FdotArt(ArtLoop,1)) > 0
            Tree{ArtLoop+2*NumGM_WM}(end+1,:) = [Vessel1(ArtLoop,7)+2*NumGM_WM,abs(FdotArt(ArtLoop,1)),-Vol1(ArtLoop)/(abs(FdotArt(ArtLoop,1))/Rho_b)];
        end
    ...
    elseif FdotArt(ArtLoop,2) >= 0 
        ...
        if abs(FdotArt(ArtLoop,2)) > 0 
            Tree{Vessel1(ArtLoop,7)+2*NumGM_WM}(end+1,:) = [ArtLoop+2*NumGM_WM,abs(FdotArt(ArtLoop,2)),Vol1(ArtLoop)/(abs(FdotArt(ArtLoop,2))/Rho_b)];
        end
...
for DomainLoop = 1:NumGM_WM
    ...
    if U(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert(I-1,J,K),Ax*Rho_b*abs(U(I,J,K)),-MTT1(I,J,K)]; end % MTT for domain already calculated in massFlowrate
    if U(I+1,J,K)>0, DownTemp(end+1,:) = [GM_WM_Convert(I+1,J,K),Ax*Rho_b*abs(U(I+1,J,K)),-MTT1(I,J,K)]; end
    if V(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert(I,J-1,K),Ay*Rho_b*abs(V(I,J,K)),-MTT1(I,J,K)]; end
    if V(I,J+1,K)>0, DownTemp(end+1,:) = [GM_WM_Convert(I,J+1,K),Ay*Rho_b*abs(V(I,J+1,K)),-MTT1(I,J,K)]; end
    if W(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert(I,J,K-1),Az*Rho_b*abs(W(I,J,K)),-MTT1(I,J,K)]; end
    if W(I,J,K+1)>0, DownTemp(end+1,:) = [GM_WM_Convert(I,J,K+1),Az*Rho_b*abs(W(I,J,K+1)),-MTT1(I,J,K)]; end

    DownTemp(end+1,:) = [GM_WM_Convert2(I,J,K),(Perfusion(I,J,K)*VoxelSize^3),-MTT2(I,J,K)];
...
for DomainLoop = 1:NumGM_WM
    ...
    if U2(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert2(I-1,J,K),Ax*Rho_b*abs(U2(I,J,K)),-MTT3(I,J,K)]; end % MTT for domain already calculated in massFlowrate
    if U2(I+1,J,K)>0, DownTemp(end+1,:) = [GM_WM_Convert2(I+1,J,K),Ax*Rho_b*abs(U2(I+1,J,K)),-MTT3(I,J,K)]; end
    if V2(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert2(I,J-1,K),Ay*Rho_b*abs(V2(I,J,K)),-MTT3(I,J,K)]; end
    if V2(I,J+1,K)>0, DownTemp(end+1,:) = [GM_WM_Convert2(I,J+1,K),Ay*Rho_b*abs(V2(I,J+1,K)),-MTT3(I,J,K)]; end
    if W2(I,J,K)<0, DownTemp(end+1,:) = [GM_WM_Convert2(I,J,K-1),Az*Rho_b*abs(W2(I,J,K)),-MTT3(I,J,K)]; end
    if W2(I,J,K+1)>0, DownTemp(end+1,:) = [GM_WM_Convert2(I,J,K+1),Az*Rho_b*abs(W2(I,J,K+1)),-MTT3(I,J,K)]; end
    ...
    if ~isempty(Volume2Vessel2{I,J,K})
        for N = 1:size(Volume2Vessel2{I,J,K},1)
            ...
            if SplitVein(DownNode,1)~=0
                DownTemp(end+1,:) = [DownNode+NumArt+2*NumGM_WM,SplitVein(N,2)*abs(Volume2Vessel2{I,J,K}(N,2)),-MTT3(I,J,K)];
                DownTemp(end+1,:) = [Vessel2(DownNode,7)+NumArt+2*NumGM_WM,(1-SplitVein(N,2))*abs(Volume2Vessel2{I,J,K}(N,2)),-MTT3(I,J,K)];
            else
                if FdotVein(DownNode,2) <= 0
                    DownNode = Vessel2(DownNode,7);
                end
                DownTemp(end+1,:) = [DownNode+NumArt+2*NumGM_WM,abs(Volume2Vessel2{I,J,K}(N,2)),-MTT3(I,J,K)];
                
            end
        end
    end
    
    Tree{DomainLoop+NumGM_WM} = DownTemp;
...
for VeinLoop = 1:NumVein  
    ...
    if FdotVein(VeinLoop,2) < 0 % && abs(FdotVein(VeinLoop,2)) > ErrorAmt
        Tree{VeinLoop+NumArt+2*NumGM_WM}(end+1,:) = [Vessel2(VeinLoop,7)+NumArt+2*NumGM_WM,abs(FdotVein(VeinLoop,2)),-Vol2(VeinLoop)/(abs(FdotVein(VeinLoop,2))/Rho_b)];
    elseif FdotVein(VeinLoop,1) > 0 % && abs(FdotVein(VeinLoop,1)) > ErrorAmt
        Tree{Vessel2(VeinLoop,7)+NumArt+2*NumGM_WM}(end+1,:) = [VeinLoop+NumArt+2*NumGM_WM,abs(FdotVein(VeinLoop,1)),Vol2(VeinLoop)/(abs(FdotVein(VeinLoop,1))/Rho_b)];
    end
...
for npnts = 1:ParticlesGenerated
    ...
    % point path function
    % choose inlet
    location = InletPoints(find(cumsum(InletFlows/sum(InletFlows))>=rand,1,'first'))+2*NumGM_WM;
    travelLog(rowcount,:) = [location(1),0];
    rowcount=rowcount+1;
    % update location
    
    
    while  ~ismember(location,OutletPoints+NumArt+2*NumGM_WM)
        ...
        % update time
        %             if location_new(1,1) >= numArt && location_new(1,1) <= (numArt + nxnynz)
        %                 fudged_MTT = location_new(1,2)R + ((2*rand)-1)*location_new(1,2);
        %             else
    ...
    % draw streamline
    if Option_Streamlines && npnts <= 500
        ...
        for n = 1:size(travelLog,1)
            ...
            if travelLog(n,1) <=2*NumGM_WM 
                [loc(n,1),loc(n,2),loc(n,3)] = ind2sub(size(GM_WM),RowConvert(travelLog(n,1)));
                loc(n,:) = loc(n,:) + [rand-0.5,rand-0.5,rand-0.5];
            elseif travelLog(n,1)<=2*NumGM_WM+NumArt
                loc(n,:) = Vessel1(travelLog(n,1)-2*NumGM_WM,3:5);
                loc(n,:) = loc(n,:)+ sph2cart(asin(2*rand-1), 2*pi*rand, (rand-0.5)*Vessel1(travelLog(n,1)-2*NumGM_WM,6)/VoxelSize);
            else
                loc(n,:) = Vessel2(travelLog(n,1)-2*NumGM_WM-NumArt,3:5);
                loc(n,:) = loc(n,:)+ sph2cart(asin(2*rand-1), 2*pi*rand, (rand-0.5)*Vessel2(travelLog(n,1)-2*NumGM_WM-NumArt,6)/VoxelSize);
            end
        end
        
        loc = [loc(:,2),loc(:,1),loc(:,3)]; % Switch x and y because of how matlab plots
        
        ArtEnd = find(travelLog(:,1)<2*NumGM_WM,1,'first')-1;
        GM_WM_End = find(travelLog(:,1)<2*NumGM_WM,1,'last')+1;
...
%             'facecol','no', 'edgecol','interp','linew',1);
    ...
    timelist(npnts,2) = sum(travelLog(1:find(travelLog(:,1)<2*NumGM_WM,1,'first')-1,2));
    timelist(npnts,3) = sum(travelLog(find(travelLog(:,1)<2*NumGM_WM,1,'first'):find(travelLog(:,1)<=2*NumGM_WM,1,'last'),2));
    timelist(npnts,4) = sum(travelLog(find(travelLog(:,1)<=(2*NumGM_WM),1,'last')+1:end,2));
...
T_TransientStore = [T_TransientStore(1:NumGM_WM,:)+T_TransientStore(NumGM_WM+1:2*NumGM_WM,:);T_TransientStore(2*NumGM_WM+1:2*NumGM_WM+NumArt,:);T_TransientStore(2*NumGM_WM+NumArt+1:end,:)];
...
for N = 1:size(T_TransientStore,2)
    T_TransientStore(1:NumGM_WM,N) = T_TransientStore(1:NumGM_WM,N)./((VoxelSize^3)*Porosity(GM_WM)); % Not 2*Porosity because it is immediately converted using only 1*Porosity in plotPerfusionTracer scripts
...
T_TransientStore = [zeros(size(T_TransientStore,1),1), T_TransientStore];
